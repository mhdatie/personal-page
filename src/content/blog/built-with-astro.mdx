...
graph TB
    User[👤 Visitor]
    CDN[Cloudflare Pages CDN<br/>Static Site Hosting]
    Form[Email Subscription Form<br/>HTML/CSS/JS]
    Turnstile[Cloudflare Turnstile<br/>Bot Protection]
    Worker[Cloudflare Workers<br/>/api/subscribe]
    Resend[Resend API<br/>Email Management]
    Secrets[Environment Variables<br/>API Tokens]

    User -->|1. Visit Site| CDN
    CDN -->|2. Serve Static Content| Form
    User -->|3. Submit Email| Form
    Form -->|4. Trigger Verification| Turnstile
    Turnstile -->|5. Verification Token| Worker
    Secrets -.->|Inject Secrets| Worker
    Worker -->|6. Validate & Forward| Resend
    Resend -->|7. Store Contact| Resend
    Worker -->|8. Response| Form
    Form -->|9. Show Confirmation| User

    style CDN fill:#f9a825
    style Worker fill:#f9a825
    style Turnstile fill:#f9a825
    style Resend fill:#4285f4
    style Secrets fill:#ea4335
    style User fill:#34a853
`} />

The architecture follows a simple serverless pattern:
- **Astro** builds the static site with Islands Architecture
- **Cloudflare Pages** serves static content from edge locations worldwide
- **Cloudflare Workers** handle serverless API logic with injected secrets
- **Turnstile** protects against bot abuse (1M free verifications/month)
- **Resend** manages email contacts with automatic deduplication (3K emails/month free)

Total cost: **$0/month** for typical personal site traffic.
